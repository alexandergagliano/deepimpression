...
from astronet.t2.attention import MultiHeadSelfAttention, MultiHeadSelfAttention_AG
...
class ConvEmbedding_AG(PrunableClusterableLayer):
    def __init__(self, num_filters, **kwargs):
        super(ConvEmbedding_AG, self).__init__(**kwargs)
        self.num_filters = num_filters
        self.conv1d = layers.Conv1D(
            filters=num_filters, kernel_size=1, activation="relu"
        )
...
class TransformerBlock_AG(PrunableClusterableLayer):
    # TODO: Update docstrings
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1, **kwargs):
        super(TransformerBlock_AG, self).__init__(**kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.ff_dim = ff_dim

        self.att = layers.MultiHeadAttention(key_dim=embed_dim, num_heads=num_heads)
        self.masking_layer = layers.Masking(mask_value=0.0)  # Adjust mask_value if needed

        #self.att = layers.MultiHeadSelfAttention_AG(num_heads=self.num_heads, key_dim=self.embed_dim)
        self.ffn = keras.Sequential(
            [
                layers.Dense(ff_dim, activation="relu"),
                layers.Dense(embed_dim),
    ...
    def call(self, inputs, training):#, mask):
        ...
        attn_output = self.att(inputs, inputs, key=inputs)  # (batch_size, input_seq_len, d_model)
        attn_output = self.dropout1(attn_output, training=training)
        out1 = self.layernorm1(
            inputs + attn_output
        )  # Residual connection, (batch_size, input_seq_len, d_model)
